...
const sections = [
  {
    ...
    items: [
      ...
      { id: 'auth-4', label: 'Existing user with incomplete profile lands on the Landing page and sees a "Complete Your Profile" banner/CTA — clicking it routes to onboarding (no automatic redirect)' },
      { id: 'auth-5', label: 'Existing user with complete profile lands on the Landing page and sees a "Start Browsing" CTA — clicking it routes to /browse (no automatic redirect)' },
      ...
      { id: 'auth-10', label: 'Onboarding Step 3 (Didit): Verify My Identity button launches Didit-hosted identity verification — this is the only verification step (skippable; no native selfie-upload requirement exists anywhere in onboarding)' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'browse-25', label: 'BROWSE-ALL: Unverified user does NOT see any "Unlock All" banner/button/dialog — only the verification gate (Verify Now) appears (covered by browse-20/browse-21)' },
      { id: 'browse-26', label: 'BROWSE-ALL: BrowseAllDialog is not reachable for unverified users — ID verification is a strict prerequisite before Browse-All is ever offered' },
      { id: 'browse-27', label: 'BROWSE-ALL: No "Verify Now" button appears inside the BrowseAllDialog because the dialog is never shown to unverified users' },
  ...
  },
  {
    ...
    items: [
      { id: 'wink-1', label: 'Browse page: Wink (😉) icon button appears on each profile card — winking is gated only by the token economy (no subscription/premium tier exists)' },
      ...
      { id: 'wink-8', label: 'PRIVACY: Wink count badge is never shown publicly on any member profile — wink counts are private and intentionally not exposed to other users' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'adm-21', label: 'ADS: When juicyads_enabled is ON, male user (show_men ON) viewing any profile — including male profiles — sees JuicyAds (no gender-pairing restriction on Profile-page ad placement; only the per-gender Show Ads toggles apply)' },
  ...
  },
  {
    ...
    color: 'text-purple-500',
    items: [
      { id: 'promo-1', label: 'Admin: /admin/bonuses-and-promos page is accessible from Admin Dashboard "Bonuses & Promos" header button' },
      { id: 'promo-2', label: 'Admin: Non-admin user navigating to /admin/bonuses-and-promos is redirected away' },
      ...
      { id: 'promo-10', label: 'Admin: Setting an expires_at date in the past causes code to be rejected when redeemed via the My Profile promo field' },
      { id: 'promo-fb8', label: 'FUNDATES: User applies FUNDATES promo code via the My Profile "Have a promo code?" field — FUNDATES promo tokens (1,000) are granted (no automatic first-purchase bonus exists — feature fully removed)' },
      { id: 'promo-fb9', label: 'FUNDATES: A TokenTransaction record of type "promo" with promo_code "FUNDATES" is created' },
      { id: 'promo-fb10', label: 'FUNDATES: Total tokens credited = pack tokens + FUNDATES promo tokens (1,000) — no additional automatic bonus is added' },
      { id: 'promo-fb11', label: 'FUNDATES: FUNDATES code cannot be redeemed a second time on the same account (used_promo_codes check)' },
      { id: 'promo-11', label: 'PURCHASE type code (e.g. FUNDATES): applying via the My Profile "Have a promo code?" field grants correct bonus tokens' },
      { id: 'promo-12', label: 'PURCHASE type code: applying the same code a second time on the same account is rejected' },
      { id: 'promo-13', label: 'PURCHASE type code: applying a VERIFICATION-type code via the My Profile promo field is rejected' },
      ...
      { id: 'promo-17', label: 'ANY type code: can be redeemed via the My Profile promo field and the verification promo field' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'pp-35', label: 'MY PROFILE: Owner views own private photos on the My Profile edit grid without any access gate or token cost (the Private Photos section is absent on the owner\'s public /profile/:id page — not gated-but-visible, simply not rendered)' },
      { id: 'pp-36', label: 'TOKEN COSTS AUDIT: Every row in the My Profile "Token Costs" list pulls its live value from the same SiteConfig fields the actual unlock/charge logic reads — no stale or hardcoded values (past issues: private-photo view showed 100 instead of 5; private-video view showed 50 instead of 200)' },
    ],
  },
  {
    id: 'admindidit',
  ...
  },
  {
    ...
    items: [
      ...
      // --- Gender targeting ---
      { id: 'ep-40', label: 'ADMIN FORM: "Gender Targeting" dropdown is present in the create/edit promo code form with options: All, Male only, Female only' },
      { id: 'ep-41', label: 'ADMIN FORM: New promo code defaults to gender="all" when created without changing the dropdown' },
      { id: 'ep-42', label: 'ADMIN FORM: Selecting "Male only" saves gender="male" to the PromoCode record' },
      { id: 'ep-43', label: 'ADMIN FORM: Selecting "Female only" saves gender="female" to the PromoCode record' },
      { id: 'ep-44', label: 'ADMIN FORM: Selecting "All" saves gender="all" to the PromoCode record' },
      { id: 'ep-45', label: 'ADMIN FORM: Editing an existing promo preserves its current gender setting in the dropdown' },
      { id: 'ep-46', label: 'ADMIN LIST: Promo codes with gender="male" show a pink "Male Only" badge in the admin list' },
      { id: 'ep-47', label: 'ADMIN LIST: Promo codes with gender="female" show a pink "Female Only" badge in the admin list' },
      { id: 'ep-48', label: 'ADMIN LIST: Promo codes with gender="all" (default) do NOT show a gender badge' },
      { id: 'ep-49', label: 'ELIGIBILITY: Male-only promo does NOT appear in EligiblePromosCard claim cards for female users' },
      { id: 'ep-50', label: 'ELIGIBILITY: Male-only promo DOES appear in EligiblePromosCard claim cards for male users' },
      { id: 'ep-51', label: 'ELIGIBILITY: Female-only promo does NOT appear in EligiblePromosCard claim cards for male users' },
      { id: 'ep-52', label: 'ELIGIBILITY: Female-only promo DOES appear in EligiblePromosCard claim cards for female users' },
      { id: 'ep-53', label: 'ELIGIBILITY: Gender="all" promo appears for both male and female users' },
      { id: 'ep-54', label: 'ELIGIBILITY: Male-only verification teaser does NOT appear for unverified female users' },
      { id: 'ep-55', label: 'ELIGIBILITY: Male-only verification teaser DOES appear for unverified male users' },
      { id: 'ep-56', label: 'SUGGESTIONS: Male-only purchase promo does NOT appear in PromoSuggestionsBanner for female users' },
      { id: 'ep-57', label: 'SUGGESTIONS: Female-only purchase promo does NOT appear in PromoSuggestionsBanner for male users' },
      { id: 'ep-58', label: 'REDEMPTION (verification): Male user applying a female-only verification/any promo code via applyVerificationPromo gets "not available for your gender" error' },
      { id: 'ep-59', label: 'REDEMPTION (verification): Female user applying a male-only verification/any promo code via applyVerificationPromo gets "not available for your gender" error' },
      { id: 'ep-60', label: 'REDEMPTION (verification): Male user applying a male-only promo code succeeds and awards tokens' },
      { id: 'ep-61', label: 'REDEMPTION (verification): Female user applying a female-only promo code succeeds and awards tokens' },
      { id: 'ep-62', label: 'REDEMPTION (purchase): Male user applying a female-only purchase promo via the My Profile "Have a promo code?" field gets "not available for your gender" error' },
      { id: 'ep-63', label: 'REDEMPTION (purchase): Female user applying a male-only purchase promo via the My Profile "Have a promo code?" field gets "not available for your gender" error' },
      { id: 'ep-64', label: 'REDEMPTION (purchase): Male user applying a male-only purchase promo via the My Profile "Have a promo code?" field succeeds' },
      { id: 'ep-65', label: 'REDEMPTION (purchase): Female user applying a female-only purchase promo via the My Profile "Have a promo code?" field succeeds' },
      { id: 'ep-66', label: 'REDEMPTION: gender="all" promo can be redeemed by both male and female users without error' },
      { id: 'ep-67', label: 'FIRST PURCHASE BONUS UI: The "First Purchase Bonus" card is NO LONGER present in Admin → Settings → Token Economy Settings' },
      { id: 'ep-68', label: 'FIRST PURCHASE BONUS REMOVED: The first-purchase bonus feature has been fully removed (both admin UI and backend) — purchasing does not grant any bonus beyond the pack\'s own token amount, regardless of SiteConfig flags' },
      // --- Award Bonuses gender selection ---
      { id: 'ep-69', label: 'AWARD BONUSES: "Gender" dropdown appears in the Award Bonuses tab ONLY when "Award to all members" is checked' },
      { id: 'ep-70', label: 'AWARD BONUSES: Gender dropdown has three options — All members, Male members only, Female members only' },
      { id: 'ep-71', label: 'AWARD BONUSES: Gender dropdown defaults to "All members" when "Award to all" is first checked' },
      { id: 'ep-72', label: 'AWARD BONUSES: Gender dropdown is NOT visible when "Award to all" is unchecked (single-member mode)' },
      { id: 'ep-73', label: 'AWARD BONUSES: Selecting "Male members only" and awarding — only male MemberProfiles receive the bonus tokens' },
      { id: 'ep-74', label: 'AWARD BONUSES: Selecting "Female members only" and awarding — only female MemberProfiles receive the bonus tokens' },
      { id: 'ep-75', label: 'AWARD BONUSES: Selecting "All members" and awarding — all MemberProfiles (male and female) receive the bonus tokens' },
      { id: 'ep-76', label: 'AWARD BONUSES: Award button text shows the filtered count — e.g. "Award Bonus to Male (N)" when male is selected' },
      { id: 'ep-77', label: 'AWARD BONUSES: Award button text shows "Award Bonus to All (N)" when "All members" is selected' },
      { id: 'ep-78', label: 'AWARD BONUSES: A TokenTransaction of type "bonus" is created for each recipient with the correct token amount and reason' },
      { id: 'ep-79', label: 'AWARD BONUSES: Success toast says "Bonus awarded to all male members successfully!" when male is selected' },
      { id: 'ep-80', label: 'AWARD BONUSES: Success toast says "Bonus awarded to all female members successfully!" when female is selected' },
      { id: 'ep-81', label: 'AWARD BONUSES: Success toast says "Bonus awarded to all members successfully!" when all is selected' },
      { id: 'ep-82', label: 'AWARD BONUSES: After awarding, the gender selection resets to "all" and the form clears' },
      { id: 'ep-83', label: 'AWARD BONUSES: Awarding to a gender filter with zero matching members shows "No members to award" error' },
    ],
  },
  {
    id: 'ads',
    ...
    items: [
      ...
      { id: 'ads-j18', label: 'JUICYADS PROFILE: Ad embed appears near the top of the page, between the "Back" link and the photo gallery' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'vid-13', label: 'PRIVATE UPLOAD: Successful video upload stores media_type="video", thumbnail_url, and correct token_cost_to_view (admin-configured tokens_private_video_cost for male uploaders)' },
      ...
      { id: 'vid-18', label: 'PRIVATE VIEW: Locked video shows blurred thumbnail with "{n} tokens to unlock" (admin-configured tokens_private_video_cost for video, tokens_private_photo_cost for photo)' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'geo-21', label: 'BROWSE: Entering a zip without selecting a country shows the same generic "Zip code not found. Please check and try again." error as an invalid zip (no distinct "no country selected" message)' },
      ...
      { id: 'geo-25', label: 'BROWSE: Collapsing and re-expanding the "Filters" toggle clears zip search, radius, country, city, and zip coords back to defaults (no dedicated "Reset" button exists)' },
